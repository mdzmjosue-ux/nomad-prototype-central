...
(() => {
  ...
  function draw(){
    ...
    const raw=clamp((scrollY-start)/travel);body.dataset.progress=raw.toFixed(4);
    // Native scroll with deliberate still poses: Web, arrival, Qualification, CRM, Systems.
    const beats=[[0,0],[.10,0],[.29,.40],[.34,.40],[.48,.57],[.56,.57],[.69,.73],[.76,.73],[.94,1],[1,1]];
    for(let i=1;i<beats.length;i++){if(raw<=beats[i][0]){const a=beats[i-1],b=beats[i];p=mix(a[1],b[1],(raw-a[0])/(b[0]-a[0]));break;}}
    body.dataset.motionProgress=p.toFixed(4);
    ...
    record.style.borderColor='#162033';
    record.style.boxShadow=`0 ${mix(14,3,system)}px ${mix(32,10,system)}px rgb(23 32 51 / ${mix(.13,.04,system)})`;
    record.querySelector('.record-edge').style.backgroundColor='#82a3ff';
    // Integer translations keep the dossier text crisp during extraction.
    record.querySelector('.record-id').style.top=`${mix(18,cheight-22,system)}px`;
    const pad=mobile?18:24;
    fields.forEach((field,i)=>{
      const a=[[pad,46],[pad,102],[pad,108],[cw*.51,108]][i];
      const bb=[[pad,46],[cw*.53,46],[pad,108],[cw*.53,108]][i];
      let end=mobile?[pad,20+i*49]:[pad+(cw-2*pad)*[0,.28,.47,.67][i],25];
      ...
      if(i===0)field.querySelector('dd').style.fontSize=`${mobile?15:mix(17,18,system)}px`;
    });
    opacity('.record-context',1-system);opacity('.record-foot',(1-enrich)*(1-system));
    $('record-context').textContent=p<.34?'Une demande à suivre':p<.49?'Reçue · À qualifier':p<.64?'Qualification':'CRM · Dossier ouvert';
    ...
    const length=$('route-active').getTotalLength();
    const firstPart=mobile?`M${points[0].join(' ')}L${points[1].join(' ')}`:`M${points[0].join(' ')} C${w*.31} ${h*.27},${w*.37} ${h*.53},${points[1].join(' ')}`;
    $('route-base').setAttribute('d',firstPart);const mid=$('route-base').getTotalLength()/length;$('route-base').setAttribute('d',path);
    const trace=p<=.57?mid*ramp(p,.40,.57):mix(mid,1,ramp(p,.57,.73));
    $('route-active').style.strokeDasharray=length;$('route-active').style.strokeDashoffset=length*(1-trace);
    const tip=$('route-active').getPointAtLength(length*trace);$('route-tracer').setAttribute('cx',tip.x);$('route-tracer').setAttribute('cy',tip.y);
    ...
    $('progress-fill').style.transform=`scaleX(${raw})`;
  }
